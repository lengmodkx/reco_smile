...
- 实时检测摄像头画面中最大的人脸。
- 根据**嘴角上扬程度**和**嘴张开比例**计算笑容分数（0 = 不笑，100 = 大笑）。
- 分数超过阈值且持续多帧后自动抓拍照片。
- 支持手动按空格键抓拍。
...
- 嘴角上扬量 `curvature`（嘴角相对唇心的上扬量，已归一化到嘴宽）：0.00 → 0 分，0.12 → 100 分
- 嘴张开比例 0.40 → 0 分，0.55 → 100 分
- 最终分数 = `0.85 × 上扬分 + 0.15 × 张开分`

> 说明：早期版本用「嘴角连线倾角」评分，但对称微笑（左右嘴角同等上扬）时连线保持水平、角度恒为 0，导致笑容几乎无法被识别。v1.0.1 起改用嘴角相对唇心的上扬量，对对称笑容同样有效。

你可以根据实际使用效果修改这些阈值和权重。调试时注意观察画面左上角的 `angle` 和 `open` 数值：

- 想让笑容更容易满分，把上扬量满分阈值从 0.12 调低（如 0.10）。
